...
The next plot examines another dimension of the missing data, namely missing date gouped by location.
...
```
Figure \ref{fig:plot_feature_selection} suggests that the accuracy peaks a few times:  with 9 predictors then 14 and tops at 22.The accuracy gain between 9 and 22 is negligible. Here is the list of features ordered by importance. We take first nine for model training.
```{r echo=FALSE}
...
```
It is counterintuitive that none of the temparture readings made it to the top, nether did our synthetic *MMDD* feature. Biases destroyed! 
