...
```{r}
selectedPredictors =  predictors(rfePrediction)[1:9]
# remove useless variables
rm(label,predictors,rfePrediction)
```
...
finalSample = weatherSample %>% select(c(selectedPredictors,"RainTomorrow"));
...
The Baive Bayes algorithm can be implemented using different packages (i.e. e1071, klaR, naivebayes) Here we are using the caret package.The caret package contains train() function which is helpful in setting up a grid of tuning parameters for a number of classification and regression routines, fits each model and calculates a resampling based performance measure.caret allows us to use the different naïve Bayes packages but in a common framework, and also allows for easy cross validation and tuning.

Here we apply NaiveBayes Model with 5-fold cross validation to have better accuracy.
...
logRegModel = train(as.formula(paste('RainTomorrow ~', selectedPredictorsPlus)),
        ...
        trControl = ctrl, preProc = c("BoxCox","center", "scale"))
...
```{r plot_model_comp,  echo=FALSE, message=FALSE, warning=FALSE, fig.align="center", fig.cap="Model Accuracy Comparison", out.width="1.1\\linewidth"}
